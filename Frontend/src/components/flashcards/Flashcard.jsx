...
const Flashcard = ({flashcard, onToggleStar}) => {
  ...
  return <div className='relative w-full h-72' style={{perspective: '1000px'}}>
    <div className={`relative w-full h-full transition-transform duration-500 transform-gpu cursor-pointer`}
        ...
        onClick={handleFlip}
    >
        {/* Font of the card (Question) */}
        <div
            className='absolute inset-0 w-full h-full bg-ehite/80 backdrop-blur-xl border-2 border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-8 flex flex-col justify-between'
            style={{
            ...
            <div className='flex items-start justify-between'>
                ...
                <button
                    ...
                    className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200
                        ${flashcard.isStarred ? 'bg-linear-to-br from-amber-400 to-yellow-500 text-white shadow-lg shadow-amber-500/25'
                ...
                >
                    <Star
                        className='w-4 h-4'
                        strokeWidth={2}
                        fill={flashcard.isFlipped ? 'currentColor' : 'none'}
                    />
                </button>
            </div>
        ...
        <div
            className='absolute inset-0 w-full h-full bg-linear-to-br from-emerald-500 to-teal-500 border-2 border-emerald-400/60 rounded-2xl shadow-xl shadow-emerald-500/30 flex flex-col justify-between'
            style={{
                backfaceVisibility: 'hidden',
                WebkitBackfaceVisibility: 'hidden',
        ...
        >
            ...
            {/* Answer Content */}
            <div className='flex-1 flex items-center justify-center px-4 py-6'>
                <p className='text-base text-white text-center loading-relaxed font-medium'>
                    {flashcard.answer}
                </p>
            </div>
